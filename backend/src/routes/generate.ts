...
router.post('/image', (req: Request, res: Response): void => {
  const { prompt, width, height, steps, seed, model } = req.body as {
    prompt?: string; width?: number; height?: number;
    steps?: number; seed?: number; model?: string;
  };
  if (!prompt) { res.status(400).json({ error: 'prompt is required' }); return; }

  const jobId = enqueue('image', { prompt, width, height, steps, seed, model }, {
    userId: req.user!.id,
    heavy: true,
  });
...
router.post('/video', (req: Request, res: Response): void => {
  ...
  const jobId = enqueue('video', {
    prompt, width: width ?? 512, height: height ?? 512, steps, seed, model,
  }, {
    userId: req.user!.id,
    heavy: true,
  });
